...
from blog_sab_app.forms import UserRegisterForm, UserEditForm
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect("blog_sab_app:user-login")
    #form = UserCreationForm()
    form = UserRegisterForm()
    return render(
        request=request,
        context={"form":form},
        template_name="blog_sab_app/register.html",
    )
...
def login_request(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return redirect("blog_sab_app:Home")

        return render(
            request=request,
            context={'form': form},
            template_name="blog_sab_app/login.html",
        )
...
def user_update(request):
    ...
    return render(
        request=request,
        context={'form': form},
        template_name="blog_sab_app/user_form.html",
    )